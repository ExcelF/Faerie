...
const std::string pathSeparator = ":";
		...
		const std::string& grammarName)
	{
		...
		if (initRecognition)
		{
			...
			s = const_cast<char*>(classPathString.c_str());
...
*/
		...
		jclass c = internal::gEnv->FindClass("voce/SpeechInterface");//internal::gClassName.c_str());
...
Make sure the class path correctly points to the Voce classes.");
		...
		jstring jStrVocePath = internal::gEnv->NewStringUTF(//"../voce-0.9.1/lib");
			vocePath.c_str());
		jstring jStrGrammarPath = internal::gEnv->NewStringUTF(//"./grammar");
			grammarPath.c_str());
		jstring jStrGrammarName = internal::gEnv->NewStringUTF(//"./grammar");
			grammarName.c_str());
